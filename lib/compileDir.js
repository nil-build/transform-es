...
module.exports = async function compileDir(
    appSrc = "src",
    appDest = "dest",
    options = {}
) {
    return glob(
        ...
        log.single(success(`转换进度: 0%`));

        return RPQueue(tasks, {
            limit: options.taskRunner,
            process: task =>
                compile(task.src, task.dest, options).then(() => {
                    n++;

                    const process = ~~((n * 100) / tasks.length);

                    log.single(success(`转换进度: ${process}% [${task.file}]`));
                })
        }).then(() => {
            log.clear();
            log.single(
                success(
                    `转换完成，耗时: ${~~((Date.now() - startTime) / 1000)} 秒`
                )
            );
            log.clear();
        });
