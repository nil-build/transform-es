...
program
	.version(pkg.version)
	.option("-w, --watch [watch]", "是否监控文件改变", false)
...
if (cliOpts.target === "node") {
	options.babel = {
		targets: {
			node: "current",
		},
	};
} else if (cliOpts.target === "esm") {
	options.babel = {
		modules: false,
	};
} else {
	options.babel = {
		modules: cliOpts.target === "web" ? "commonjs" : cliOpts.target,
	};
}
...
transformEs(src, cliOpts.outDir, options);
