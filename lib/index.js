...
module.exports = async function (appSrc = "src", outDir = "lib", options = {}) {
	...
	useLog && log(success(`任务数：${fileTotal}`));
	...
	RPQueue(files, {
		...
		process: file => {
			...
			p.finally(() => {
				...
				useLog && log.update(success(`转换进度：${~~process}%`));
			});

			return p;
		},
	}).finally(() => {
		useLog && log.update(success(`转换完成，耗时：${~~((Date.now() - startTime) / 1000)} 秒`));

		useLog && log.flush();

		if (options.watch) {
			useLog && log("%s文件夹监听中...", success(appSrc));
			watcher(options);
		}
	});
};
